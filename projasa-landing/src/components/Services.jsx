...
export default function Services() {
  ...
  return (
    <section id="harga" className="relative py-24 overflow-hidden bg-gradient-to-b from-white via-[#ecf4f8] to-[#f4f0fd]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredLayanan.map((item, idx) => (
            <div
              key={idx}
              className="group relative bg-white/50 backdrop-blur-md rounded-[24px] border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden"
            >
              {/* Image */}
              <div className="w-full h-48 overflow-hidden relative">
                ...
                />
                {/* Category badge */}
                <span className={`absolute top-3 left-3 px-3 py-1 rounded-full text-[10px] font-bold border ${categoryColors[item.category] || 'bg-slate-50 text-slate-600 border-slate-200'}`}>
                  {item.category}
                </span>
              </div>
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-2 mb-5 pb-4 border-b border-slate-100">
                  <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Mulai dari</span>
