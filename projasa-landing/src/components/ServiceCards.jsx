...
export default function ServiceCards({ onViewCompany, onViewServices }) {
  return (
    <section id="layanan" className="pt-4 sm:pt-8 pb-24 sm:pb-32 w-full bg-gradient-to-b from-[#f4f0fd] to-white relative">
      
      {/* === MOBILE LAYOUT: Horizontal cards with sticky overlap === */}
      <div className="md:hidden px-4 flex flex-col gap-0">
        {companies.map((company, index) => (
          <div
            key={company.name}
            className={`sticky top-16 rounded-2xl overflow-hidden flex flex-row items-stretch shadow-lg border ${
              company.dark
                ? 'bg-[#0a0a0b] border-slate-800'
                : `${company.bgGradient} border-slate-100`
            }`}
            style={{ zIndex: (index + 1) * 10, marginBottom: index < companies.length - 1 ? '-8px' : '0' }}
          >
            {/* Left: Image */}
            ...
            <div className="flex-1 p-4 flex flex-col justify-center relative z-10">
              <p className={`text-[9px] font-bold uppercase tracking-widest mb-1 ${company.dark ? 'text-slate-400' : 'text-slate-600'}`}>
                {company.tagline}
              </p>
              <h3 className={`text-[15px] font-black tracking-tight leading-tight mb-2 ${company.dark ? 'text-white' : 'text-slate-900'}`}>
              ...
              <div className="flex flex-wrap gap-1 mb-3">
                ...
                {company.services.length > 3 && (
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${
                    company.dark ? 'text-white/50' : 'text-slate-500'
                  }`}>
                    +{company.services.length - 3}
                  </span>
                )}
              ...
              <button
                ...
                className={`self-start px-4 py-1.5 rounded-full text-[11px] font-bold transition-colors cursor-pointer ${
                  company.dark
                    ? 'border border-white/60 text-white bg-transparent'
                    : 'border border-slate-900/60 text-slate-900 bg-transparent'
                }`}
              >
                Lihat Layanan
              </button>
      ...
      {/* === DESKTOP LAYOUT: Original large sticky cards === */}
      <div className="hidden md:flex px-6 max-w-7xl mx-auto flex-col gap-[50vh]">
        {companies.map((company, index) => (
          <div
            key={company.name}
            className={`relative sticky top-20 rounded-[2rem] overflow-hidden flex flex-col lg:flex-row items-center p-10 md:p-16 gap-12 shadow-lg border ${
              company.dark
      ...
      {/* Bottom CTA */}
      <div className="flex flex-row justify-center gap-2 sm:gap-4 mt-16 sm:mt-20 px-4 sm:px-6">
        <button
          onClick={onViewServices}
          className="inline-flex items-center justify-center gap-1.5 sm:gap-2 flex-1 sm:flex-none px-4 sm:px-8 py-3.5 sm:py-4 rounded-full bg-[#c892ff] hover:bg-[#b87df8] text-white font-bold text-xs sm:text-base transition-all duration-300 transform hover:-translate-y-1 cursor-pointer shadow-lg"
        >
          Lihat Semua Layanan
        ...
        <a
          ...
          className="inline-flex items-center justify-center gap-1.5 sm:gap-2 flex-1 sm:flex-none px-4 sm:px-8 py-3.5 sm:py-4 rounded-full bg-[#25D366] text-white font-bold text-xs sm:text-base hover:bg-[#20BA5A] transition-all duration-300 transform hover:-translate-y-1 shadow-lg"
        >
